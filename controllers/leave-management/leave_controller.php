...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
 ...
 if(isset($_POST['add-leave-details']))
{
...
$sql_insert= mysqli_query($dbc,"INSERT INTO leave_details
      (email, leave_name, reliever, entitled_days, start_date, end_date, duration,reason,  date_recorded,time_recorded)
        VALUES ('".$_SESSION['email']."','".$leave_name."','".$reliever."','".$entitled_days."', '".$start_date."', '".$end_date."','".$duration."','".$reason."',
          '".$date_recorded."',  '".$time_recorded."')") or die (mysqli_error($dbc));


//log the action
$action_reference = "Added Leave " . $leave_name;
$action_name = "Leave Information Creation";
...
else if(isset($_POST['approve_leave']))
{
  /* set autocommit to off */
  mysqli_autocommit($dbc, FALSE);
  ...
  $change_status = mysqli_query($dbc,"UPDATE leave_details SET status='$status' WHERE id='".$id."'");

  //log the action
  $action_reference = "Approved the leave with id " . $id. " to ".$status;
  $action_name = "Leave Status Change";
...
else if(isset($_POST['add-leave-class-details']))
{
  $leave_name = mysqli_real_escape_string($dbc,strip_tags($_POST['leave_name']));
$entitled_days = mysqli_real_escape_string($dbc,strip_tags($_POST['entitled_days']));
...
$sql_insert= mysqli_query($dbc,"INSERT INTO leave_type
    (leave_name, entitled_days, gender, recorded_by)
      VALUES ('".$leave_name."','".$entitled_days."','".$gender."', '".$_SESSION['email']."')") or die (mysqli_error($dbc));


//log the action
$action_reference = "Added Leave Class Details" . $bank_name;
$action_name = "Leave class Information Creation";
$action_icon = "far fa-project-diagram text-success";
$page_id = "Job Seeker Application Details";
$time_recorded = date('Y/m/d H:i:s');
