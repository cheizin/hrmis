...
*/
 ...
 $profile_pic = mysqli_fetch_array(mysqli_query($dbc,"SELECT * FROM staff_users"));
?>
<nav aria-label="breadcrumb">
     <ol class="breadcrumb">
       <li class="breadcrumb-item active" aria-current="page">Candidates Management</li>
     </ol>
</nav>
  ...
  <div class="col-lg-12 col-xs-12">
    <div class="card card-primary card-outline">
      ...
      <div class="card-body table-responsive">
        ...
        <?php
        $sql_query1 =  mysqli_query($dbc,"SELECT * FROM staff_users WHERE access_level ='job-seeker' ORDER BY id");
       ...
       <table class="table table-striped table-bordered table-hover" id="invoice-received-table"  style="width:100%">
         ...
         <?php
         $no = 1;
          $sql = mysqli_query($dbc,"SELECT * FROM staff_users WHERE access_level ='job-seeker' ORDER BY id DESC");
         ...
         <tr style="cursor: pointer;">
           ...
           <td> <b><?php echo $row['lName'] ;?> <?php echo $row['fName'] ;?> </b> <br/>Worked/Works at <b><?php echo $row['companyName'] ;?></b> <br/>
            as a <b> <?php echo $row['currentPosition'] ;?> </b> <br/>
               with <b> <?php echo $row['experience'] ;?> </b> <br/>
               Experience

           </td>
            <td>
                <?php
              $sql_query1 =  mysqli_query($dbc,"SELECT * FROM education_history WHERE email ='".$row['Email']."'");
              $number = 1;
              if($total_rows1 = mysqli_num_rows($sql_query1) > 0)
              {?>
...
Schooled at <b>
              <?php
                   ...
                   $result = mysqli_query($dbc, "SELECT * FROM education_history WHERE email ='".$row['Email']."' ORDER BY id DESC LIMIT 1"  );
                   if(mysqli_num_rows($result))
                   {
                     while($profile_pic = mysqli_fetch_array($result))
                     {
                       ?>
                       <?php echo $profile_pic['school_name']; ?>
                   ...
                   <?php
                        ...
                        if(mysqli_num_rows($result))
                        {
                          ...
                          {
                            ?>
                            <?php echo $profile_pic['qualification']; ?>
...
For the Course <b>
                        <?php
                             ...
                             if(mysqli_num_rows($result))
                             {
                               ...
                               {
                                 ?>
                                 <?php echo $profile_pic['qualification_name']; ?>
             ...
             <td>
               <a class="" href="#" data-toggle="modal" data-target="#application-request-modal-<?php echo $row['id'];?>"
                 title="Click on <?php echo $row['invoice_received_id'];?> to update Invoice payments for the invoice">
                 <span class="text-primary" style="cursor:pointer;">Request To Apply</span>
               </a>

               <!-- start invoice payment Modal -->
               <div class="modal fade" id="application-request-modal-<?php echo $row['id'];?>" role="dialog">
                 <div class="modal-dialog" role="document">
                   <div class="modal-content">
                     <div class="modal-header alert alert-success">
                       <h5>Request <b><?php echo $row['lName'];?></b> To Apply For the position Below:
                     ...
                     <div class="modal-body">
                             <form id="request-application-form-<?php echo $row['id'];?>" onsubmit="SubmitRequest('<?php echo $row['id'];?>');"
                               class="mt-4" enctype="multipart/form-data">

                               <input type="hidden" name="email" id="email-<?php echo $row['id'];?>"  value="">
                            ...
                            <textarea name="special_info" id="special_info-<?php echo $row['id'];?>" class="form-control" required></textarea>

                               </div>

                                                              <div class="col-lg-12 col-xs-12 form-group">
                                                                <label for="category"><span class="required">*</span>Job Post</label>
                                                                <select name="post_name" id="post_name-<?php echo $row['id'];?>" class="select2 form-control" data-placeholder="Select Post Name">
                                                                    <?php
                                                                      $result = mysqli_query($dbc, "SELECT * FROM job_posting");
